...
Приложение использует rate limiting для предотвращения злоупотреблений. Текущие лимиты:

- **Глобальные лимиты:** 200 запросов в день, 50 запросов в час на пользователя.
- **Эндпоинт `/api/raid/start`:** 1 запрос в минуту.
- **Эндпоинт `/api/raid/dig`:** 20 запросов в минуту.

Если вы получаете ошибку 429 (Too Many Requests), подождите некоторое время перед повторной попыткой. Разработчики могут корректировать лимиты в коде (`app.py`) и переразвернуть приложение на Railway.

Для пользователей: избегайте быстрого нажатия кнопок в игре, чтобы не превысить лимиты.
Для разработчиков: мониторьте логи на Railway на предмет 429 ошибок и корректируйте лимиты при необходимости.
