...
def migrate_tables():
    ...
    try:
        ...
        # Добавить столбец is_setup_complete, если его нет
        cursor.execute('ALTER TABLE users ADD COLUMN IF NOT EXISTS is_setup_complete BOOLEAN DEFAULT FALSE')
        conn.commit()
...
def onboard():
    ...
    # Валидация username: длина 3-12, только буквы, цифры, _
    import re
